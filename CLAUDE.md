...
- **Corpus build:** `generate_all.py` is the one entrypoint (wraps the sub-generators; `--dry-run`, `--skip-teacher/--skip-hf/--skip-fullfix`). Trajectory shapes: short stylized snippets (`generate_agentic_data.py`, 1–4 calls) + **full-fix** long end-to-end task runs (`generate_fullfix_data.py` — explore→reason→edit→verify→summarize over many iterations; four outcomes incl. failures + iteration budget + forced summary; mirrors `lambda/hermes-agent-reasoning-traces`). See `docs/python/fullfix-trajectory-generation.md`.
- **Schema-free tool calling:** `build_dataset.py --embed-tools-frac` (default 0.25) renders most CORE-catalog trajectories with `tools=None` (native `tool_calls` kept) so the model recognizes the stable catalog from training, not an embedded schema block per prompt; a slice + all non-core/HF/dynamic-MCP rows stay embedded. Serving needs no change (serve_chat does `tools=tools or None`). Verify the tokenizer renders calls with `tools=None` before a run — see `docs/python/tool-internalization.md`.
- **Trainers** (all bf16, instruct base `Qwen/Qwen3.5-9B`, packing + fla; env knobs `BATCH_SIZE/GRAD_ACCUM/GRAD_CKPT/PACK/DL_WORKERS/EPOCHS/LR`):
  ...
  - `spark/train_molf.py` + `spark/molf.py` — **MoLF-E** (arXiv:2605.07111): frozen base + 2 LoRA experts (different ranks via `RANKS`) routed by a custom Sparse-AdamW (EPD Top-1 per module); folds to a standard LoRA adapter on export. **Best results** at `RANKS=128,256 ALPHA=32` (capacity is the lever — see adapters). Must freeze the full base before injecting (else ~4B params keep grad). Standard MoLF (FFT expert) OOMs the 9B → only MoLF-E fits.
...
- **Serving:** `ADAPTER_NAME=<name> spark/serve_chat.sh start` — transformers-backed OpenAI server on `:8765` (vLLM v0.21 can't load `qwen3_5`); serves model names `lamark` (base+adapter) and `base`; local REPL `chat_lamark.py` connects unchanged. serve_chat pins `device_map={"":0}` and stops at `<|im_end|>` (Qwen3.5's eos is `<|endoftext|>`, so without that it invents a follow-up turn). `serve_vllm.sh` is the NemotronH-era path only.
- **Adapters** (`~/.lamark/checkpoints/`), best last: `qwen3_5-9b-instruct-tools-lora` (id/knowledge 100, tools 75); `qwen3_5-9b-agentic-lora` (native tool-calls but identity regressed — **undertrained**: packing cut it to ~28 steps); **`qwen3_5-9b-molf-lora-v2` — current best, perfect gate (identity/knowledge/tools/regression all 100%) + native tool-calls** (MoLF-E r=128/256). **Watch undertraining:** packing collapses to few optimizer steps — raise `EPOCHS` (a 2-epoch packed run undertrained identity/knowledge). **Capacity helped:** only higher-rank experts reached tools 100% (r=32 LoRA capped at 75%).
...
24. **Never use greedy decoding with any Gemma 4 model.** They inherit the same greedy-loop bug as Qwen3. Use `temperature=0.7, top_p=0.9, top_k=40` (tool loops, thinking off) or `temperature=0.6, top_p=0.95, top_k=40` (reasoning, thinking on). Applies to serving, batch eval, and synthetic data generation.
25. **Gemma 4 LoRA target modules differ** from standard Transformer — the hybrid attention architecture (local sliding window + global final layer with p-RoPE) uses different projection names (`q_proj`, `v_proj` at global layers differ from sliding-window layers). Verify `FastModel.from_pretrained` accepts the model and `model.named_modules()` against the LoRA config before committing a training run.
26. **Load Gemma 4 12B for inference with `device_map={"":0}`.** The encoder-free (12B Unified) variant projects raw image/audio patches directly into the LLM embedding space via lightweight linear layers — `"auto"` may offload these to CPU causing inference failures. The 26B/31B variants use dedicated vision encoders (~550M params) and need sufficient GPU memory for both the decoder and encoder.
