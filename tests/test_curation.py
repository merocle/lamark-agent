...
@pytest.mark.parametrize("asst", [
    ...
    "the ask_cloud tool is not found in this session",
    "",
])
...
def test_regex_drops_triage_injection():
    rec = {"messages": [
        {"role": "user", "content": "[This question is hard - do NOT answer ...] derive equations"},
...
def test_regex_keeps_good_pair():
    rec = {"messages": [
        ...
        {"role": "assistant", "content": "Rayleigh scattering - short wavelengths scatter more."},
    ]}
...
def test_plan_drops_garbage_keeps_good(tmp_path, monkeypatch):
    ...
    _write(arch, "why is the sky blue", "Rayleigh scattering.", evidence="telegram:s1")
    _write(arch, "x", "Tool 'ask_cloud' does not exist. Available tools: terminal", evidence="telegram:s1")
    _write(arch, "who are you", "I'm Lamark.", evidence="quality-seed:gold-handcrafted")
    ...
    assert "Rayleigh scattering." in contents
    assert "I'm Lamark." in contents
    assert all("does not exist" not in c for c in contents)


def test_plan_judge_drops_pair(tmp_path, monkeypatch):
    monkeypatch.setenv("LAMARK_HOME", str(tmp_path))
    arch = _mk_archive(tmp_path)
    _write(arch, "good question", "good answer", evidence="telegram:s1")
    _write(arch, "bad question", "bad answer", evidence="telegram:s1")

    def fake_judge(rec, *, model):
        return not rec["messages"][0]["content"].startswith("bad")
    ...
    assert "good question" in users
    assert "bad question" not in users
