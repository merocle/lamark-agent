...
def test_prefilter_explicit_cloud():
    # Anchored on cloud model names, so it fires in any language as long as
    # the name is present in Latin form.
    for t in ["ask Claude about entropy", "use opus for this",
              "could you ask GPT", "run this through gemini"]:
        assert C._regex_prefilter(t) == "explicit_cloud", t


def test_prefilter_casual():
    for t in ["hi", "thanks!", "ok", "hello", "yo", "cool"]:
        assert C._regex_prefilter(t) == "casual", t


def test_prefilter_short_question_is_not_casual():
    assert C._regex_prefilter("why?") is None
    assert C._regex_prefilter("who is this?") is None
...
def test_classify_shortcircuits_without_llm(monkeypatch):
    ...
    assert C.classify("hello")["intent"] == "casual"
    assert C.classify("ask Claude X")["intent"] == "explicit_cloud"
    assert called["n"] == 0
...
def test_classify_fallback_on_llm_failure(monkeypatch):
    ...
    out = C.classify("when was Lamarck born")
...
def test_factual_injects_grounding(monkeypatch):
    ...
    ev = _FakeEvent(text="when was Jean-Baptiste Lamarck born")
    R.apply(ev, model="lamark")
...
def test_factual_web_failure_injects_caveat(monkeypatch):
    ...
    ev = _FakeEvent(text="current USD rate")
    R.apply(ev, model="lamark")
    assert ev.channel_prompt is not None
...
def test_empty_web_results_injects_caveat(monkeypatch):
    ...
    ev = _FakeEvent(text="some obscure fact")
    R.apply(ev, model="lamark")
    assert "uncertainty caveat" in (ev.channel_prompt or "")


# -- router: needs_cloud -> escalation injected into user text -------
...
def test_needs_cloud_injects_escalation_into_text(monkeypatch):
    ...
    ev = _FakeEvent(text="derive the Einstein field equations")
    R.apply(ev, model="lamark")
    assert "ask_cloud" in ev.text
    assert "claude-opus-4-5" in ev.text
    assert ev.text.rstrip().endswith("derive the Einstein field equations")
...
def test_reasoning_without_needs_cloud_stays_local(monkeypatch):
    ...
    ev = _FakeEvent(text="prove sqrt(2) is irrational")
    R.apply(ev, model="lamark")
    assert ev.text == "prove sqrt(2) is irrational"
...
def test_noop_intents_leave_event_untouched(monkeypatch, intent):
    monkeypatch.setattr(R, "classify",
                        lambda t, **k: {"intent": intent, "needs_web": False, "needs_cloud": False})
    ev = _FakeEvent(text="whatever")
    R.apply(ev, model="lamark")
    assert ev.channel_prompt is None
    assert ev.text == "whatever"
