...
| **Gemma 4 12B Unified IT** (`gemma4_unified_text` arch — dense, encoder-free) | 12B dense | 48 | 256K | **Yes** — full-capability agent; strong reasoning + multimodal. Hybrid local-global attention (1024 sliding window + global final layer). Text, Image, Audio natively. p-RoPE. 262K vocab. | BF16 or Q8_0 | 32–40 GB | Unsloth LoRA r=32–64; ~14–20 GB (BF16) / ~8–12 GB (QLoRA) |

> **Deploy:** Qwen3.5 all four via `llama-server --jinja` (llama.cpp sm_121 build for Spark). Gemma 4 via vLLM / SGLang / llama.cpp. Serving params: `--temp 0.7 --top-p 0.8 --top-k=20` (Qwen3.5 tool loops, **never greedy** — Qwen3.5 inherits the Qwen3 greedy-loop bug); `--temp 0.7 --top-p 0.9 --top-k=40` (Gemma 4 tool loops, **never greedy**). Thinking: `enable_thinking=True/False` (Gemma 4) / ````thinking```` blocks (Qwen3.5). Gemma 4 reasoning channel — start token `<|channel>` (followed by the channel name `thought`), end token `<channel|>` (reversed pipe), then the final answer; Qwen3.5 uses `<think>…</think>`. (These per-family delimiters live in `learning/scripts/model_template.py` for the training/eval side.)
