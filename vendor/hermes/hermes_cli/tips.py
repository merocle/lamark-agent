...
TIPS = [
    ...
    "lamark chat -t web,terminal enables only specific toolsets for a focused session.",
    "lamark chat -s github-pr-workflow preloads a skill at launch.",
    "lamark chat -q \"query\" runs a single non-interactive query and exits.",
    "lamark chat --max-turns 200 overrides the default 90-iteration limit per turn.",
    "lamark chat --checkpoints enables filesystem snapshots before every destructive file change.",
    "hermes --yolo bypasses all dangerous command approval prompts for the entire session.",
    "lamark chat --source telegram tags the session for filtering in lamark sessions list.",
    "hermes -p work chat runs under a specific profile without changing your default.",

    # --- CLI Subcommands ---
    "lamark doctor --fix diagnoses and auto-repairs config and dependency issues.",
    "lamark dump outputs a compact setup summary — great for bug reports.",
    "lamark config set KEY VALUE auto-routes secrets to .env and everything else to config.yaml.",
    "lamark config edit opens config.yaml in your default editor.",
    "lamark config check scans for missing or stale configuration options.",
    "lamark sessions browse opens an interactive session picker with search.",
    "lamark sessions stats shows session counts by platform and database size.",
    "lamark sessions prune --older-than 30 cleans up old sessions.",
    "lamark skills search react --source skills-sh searches the skills.sh public directory.",
    "lamark skills check scans installed hub skills for upstream updates.",
    "lamark skills tap add myorg/skills-repo adds a custom GitHub skill source.",
    "lamark skills snapshot export setup.json exports your skill configuration for backup or sharing.",
    "lamark mcp add github --command npx adds MCP servers from the command line.",
    "lamark mcp serve runs Hermes itself as an MCP server for other agents.",
    "lamark auth add lets you add multiple API keys for credential pool rotation.",
    "lamark completion bash >> ~/.bashrc enables tab completion for all commands and profiles.",
    "lamark logs -f follows agent.log in real time. --level WARNING --since 1h filters output.",
    "lamark backup creates a zip backup of your entire Hermes home directory.",
    "lamark profile create coder creates an isolated profile that becomes its own command.",
    "lamark profile create work --clone copies your current config and keys to a new profile.",
    "lamark update syncs new bundled skills to ALL profiles automatically.",
    "lamark gateway install sets up Hermes as a system service (systemd/launchd).",
    "lamark memory setup lets you configure an external memory provider (Honcho, Mem0, etc.).",
    "lamark webhook subscribe creates event-driven webhook routes with HMAC validation.",
    "Save money: lamark tools disables unused tools, lamark skills config trims skills down.",
    "/reasoning low or /reasoning minimal cuts thinking depth below the default (medium) — faster, cheaper responses.",
    "lamark models routes vision, compression, and aux tasks to cheaper models — cuts background token cost 85%+ without downgrading your main chat model.",
    ...
    "Profile names become shell commands — 'lamark profile create coder' creates the 'coder' command.",
    "lamark profile export coder -o backup.tar.gz creates a portable profile archive.",
    ...
    "lamark sessions export backup.jsonl exports all sessions for backup or analysis.",
    ...
    "lamark skills install official/security/1password installs optional skills from the repo.",
    ...
    # --- Cron & Scheduling ---
    "Cron jobs can attach skills: lamark cron add --skill blogwatcher \"Check for new posts\".",
    ...
    "lamark gateway install sets it up as a system service that starts on boot.",
    ...
    "SOUL.md at $HERMES_HOME/SOUL.md is the agent's primary identity — customize it to shape behavior.",
    ...
    "MCP servers auto-generate toolsets at runtime — lamark tools can toggle them per platform.",
    ...
    "Checkpoints use shadow repos in $HERMES_HOME/checkpoints/ — your project's .git is never touched.",

    # --- Batch & Data ---
    "batch_runner.py processes hundreds of prompts in parallel for training data generation.",
    "lamark chat -Q enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    "lamark plugins install owner/repo installs plugins directly from GitHub.",
    ...
    "lamark acp runs Hermes as an ACP server for VS Code, Zed, and JetBrains integration.",
    ...
    "lamark login supports OAuth-based auth for Nous and OpenAI Codex providers.",
    ...
    "lamark status --deep runs deeper diagnostic checks across all components.",
    ...
    "Cron scripts live in $HERMES_HOME/scripts/ and run before the agent — perfect for data collection pipelines.",
    ...
    "SOUL.md is auto-seeded with a default personality on first run. Edit $HERMES_HOME/SOUL.md to customize.",
    ...
    "The skills audit log at $HERMES_HOME/skills/.hub/audit.log tracks every install and removal operation.",
    ...
    "Every interrupt during an agent run is logged to $HERMES_HOME/interrupt_debug.log with timestamps.",
    ...
    "Profile names cannot collide with existing PATH binaries — 'lamark profile create ls' would be rejected.",
    "lamark profile create backup --clone-all copies everything (config, keys, SOUL.md, memories, skills, sessions).",
    ...
    "lamark claw migrate --dry-run previews OpenClaw migration without writing anything.",
    ...
    "Each MCP server gets its own toolset (mcp-servername) that can be toggled independently via lamark tools.",
    "MCP ${ENV_VAR} placeholders in config are resolved at server spawn — including vars from $HERMES_HOME/.env.",
    "Skills from trusted repos (NousResearch) get a 'trusted' security level; community skills get extra scanning.",
    "The skills quarantine at $HERMES_HOME/skills/.hub/quarantine/ holds skills pending security review.",
    ...
    '/reload reloads $HERMES_HOME/.env into the running session — pick up new API keys without restarting.',
    ...
    # --- Gateway Hooks ---
    'Gateway hooks live under $HERMES_HOME/hooks/<name>/ with HOOK.yaml + handler.py — handler must be named `handle`.',
    'Hook events include gateway:startup, session:start, agent:step, and command:* wildcard subscriptions.',
    'Drop a $HERMES_HOME/BOOT.md checklist and a gateway:startup hook runs it as a one-shot agent every boot.',

    # --- Curator ---
    'lamark curator run --dry-run previews what the curator would archive or consolidate without mutating anything.',
    "lamark curator pin <skill> hard-fences a skill against both auto-archival and the agent's skill_manage tool.",
    'lamark curator rollback restores skills from a pre-run snapshot — backups live under skills/.curator_backups/.',

    # --- Credential Pools & Routing ---
    'lamark auth reset <provider> clears all cooldowns and exhaustion flags on a credential pool.',
    ...
    'lamark dashboard launches a local web UI at 127.0.0.1:9119 — zero data leaves localhost.',
    'lamark dashboard --tui embeds the full Hermes TUI in your browser via xterm.js and a WebSocket PTY.',
    'Drop a YAML in $HERMES_HOME/dashboard-themes/ with two palette colors to reskin the entire dashboard.',
    'Dashboard plugins are drop-in: manifest.json + JS bundle in $HERMES_HOME/dashboard-plugins/ — no npm build required.',
    ...
    # --- TTS ---
    'tts.provider: piper runs 44-language local TTS on CPU — voices auto-download to $HERMES_HOME/cache/piper-voices/.',
    ...
    '/reload-skills re-scans $HERMES_HOME/skills/ so drop-in skills appear without restarting the session.',
    ...
    'lamark chat --pass-session-id injects the session ID into the system prompt so the agent can self-reference it.',
    'lamark chat --image path/to/pic.png attaches a local image to a single -q query without a separate upload step.',
    'lamark chat --ignore-user-config skips $HERMES_HOME/config.yaml — reproducible bug reports and CI runs.',
    "lamark chat --source tool tags programmatic chats so they don't clutter lamark sessions list.",
    'lamark dump --show-keys includes redacted API key fingerprints for deeper support debugging.',
    'lamark sessions rename <ID> "new title" renames any past session; lamark sessions delete <ID> removes one.',
    'lamark import restores a session export or profile archive produced by sessions export or profile export.',
    'lamark fallback manages the fallback_model chain interactively — no hand-editing config.yaml.',
    'lamark pairing rotates the DM pairing token — the first messager after rotation claims access to the bot.',
    'lamark setup walks first-time users through provider, keys, and platform wiring in one interactive flow.',
    'lamark status --deep runs the full health sweep across every component; plain lamark status is the quick view.',
    ...
    '--source tool chats are excluded from lamark sessions list by default — set --source explicitly to see them.',
    ...
    'Dashboard plugins are served from /dashboard-plugins/<name>/ — drop files into $HERMES_HOME/dashboard-plugins/.',
]
