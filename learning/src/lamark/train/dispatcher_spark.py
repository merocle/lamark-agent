...
def main() -> int:
    ...
    parser.add_argument("--model-registry-name", type=str, default=None,
                        help="Lookup LoRA targets from scripts/model-registry.yaml "
                             "by this name (e.g. 'qwen-3.6-35b-a3b-moe'). If unset, "
                             "use CLI overrides + dense defaults.")
    parser.add_argument("--lora-rank", type=int, default=None)
    parser.add_argument("--lora-alpha", type=int, default=None)
    parser.add_argument("--lora-target-modules", type=str, default=None,
                        help="Comma-separated LoRA target modules (overrides registry).")
    ...
    # Registry-driven LoRA config — pick defaults from model-registry.yaml,
    # let CLI args override. This makes the dispatcher model-agnostic: same
    # script works for MoE (attention-only) and dense (all-linear) models.
    lora_rank = args.lora_rank if args.lora_rank is not None else 16
    lora_alpha = args.lora_alpha if args.lora_alpha is not None else 32
    lora_targets: list[str] | None = None
    if args.lora_target_modules:
        lora_targets = [t.strip() for t in args.lora_target_modules.split(",") if t.strip()]
    if args.model_registry_name:
        try:
            from lamark.registry import get_model as _get_model
            entry = _get_model(args.model_registry_name)
            if args.lora_rank is None:
                lora_rank = entry.lora.rank
            if args.lora_alpha is None:
                lora_alpha = entry.lora.alpha
            if lora_targets is None:
                lora_targets = list(entry.lora.target_modules)
        except (ImportError, KeyError):
            pass
    if lora_targets is None:
        # Fallback: attention-only — safe for both MoE and dense.
    ...
    # 4. Attach LoRA — targets and rank come from registry or CLI overrides
    try:
        lora_cfg = LoraConfig(
            r=lora_rank,
        ...
        report.notes.append(f"LoRA config: rank={lora_rank}, alpha={lora_alpha}, "
